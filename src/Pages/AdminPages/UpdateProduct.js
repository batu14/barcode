      ...
      <Tab as="span">
        {({ hover, selected }) => (
          <a
            className={
              selected
                ? "bg-indigo-600 block text-sm  text-center lg:text-base px-6 py-2 text-white rounded-md"
                : "bg-indigo-400  block text-center text-sm lg:text-base px-6 py-2 text-white rounded-md"
            }
          >
            {children}
          </a>
  ...
  useEffect(() => {
    ...
    fetch(process.env.REACT_APP_BASE_URL + "product.php", {
      ...
      .then((data) => {
        ...
        setCost(data.cost);
        setDate(data.date);
        data.list == 1 ? setList(true) : setList(false);
      })
      .catch((err) => {
        console.log(err);
      });
    ...
    <Container padding>
      ...
      <div className="mb-6">
        ...
        <p className="text-gray-600">
          {barcode} barkodlu ürünü düzenliyorsunuz
        </p>
      </div>
                ...
                <div className="flex items-center justify-between">
                  <label className="flex items-center gap-3">
                    ...
                    <Switch
                      ...
                      onChange={setList}
                      className={`${
                        list ? "bg-indigo-600" : "bg-gray-200"
                      } relative inline-flex h-6 w-11 items-center rounded-full transition-all`}
                    >
                      <span
                        className={`${
                          list ? "translate-x-6" : "translate-x-1"
